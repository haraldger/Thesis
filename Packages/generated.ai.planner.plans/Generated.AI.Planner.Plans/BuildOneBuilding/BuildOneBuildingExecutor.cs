...
namespace Generated.AI.Planner.Plans.BuildOneBuilding
{
    ...
    public struct TerminationEvaluator : ITerminationEvaluator<StateData>
    {
        public bool IsTerminal(StateData state, out float terminalReward)
        {
            ...
            var BuildingBuiltInstance = new BuildingBuilt();
            if (BuildingBuiltInstance.IsTerminal(state))
            {
                terminal = true;
                terminalReward += BuildingBuiltInstance.TerminalReward(state);
            }
            return terminal;
        }
    }

    class BuildOneBuildingExecutor : BaseTraitBasedPlanExecutor<TraitBasedObject, StateEntityKey, StateData, StateDataContext, StateManager, ActionKey>
    {
        ...
        public  BuildOneBuildingExecutor(StateManager stateManager, PlannerStateConverter<TraitBasedObject, StateEntityKey, StateData, StateDataContext, StateManager> stateConverter)
        {
