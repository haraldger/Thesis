...
namespace Generated.AI.Planner.Plans.SimpleBuildingProblem
{
    ...
    public struct TerminationEvaluator : ITerminationEvaluator<StateData>
    {
        public bool IsTerminal(StateData state, out float terminalReward)
        {
            ...
            var BarracksBuiltInstance = new BarracksBuilt();
            if (BarracksBuiltInstance.IsTerminal(state))
            {
                terminal = true;
                terminalReward += BarracksBuiltInstance.TerminalReward(state);
            }
            return terminal;
        }
    }

    class SimpleBuildingProblemExecutor : BaseTraitBasedPlanExecutor<TraitBasedObject, StateEntityKey, StateData, StateDataContext, StateManager, ActionKey>
    {
        ...
        public  SimpleBuildingProblemExecutor(StateManager stateManager, PlannerStateConverter<TraitBasedObject, StateEntityKey, StateData, StateDataContext, StateManager> stateConverter)
        {
        ...
        protected override void Act(ActionKey actionKey)
        {
            ...
            foreach (var argument in argumentMapping)
            {
                ...
                if (split.Length > 1) // argument is a trait
                {
                    switch (split[1])
                    {
                        case nameof(BuildingSpot):
                            var traitBuildingSpot = stateData.GetTraitOnObjectAtIndex<BuildingSpot>(traitBasedObjectIndex);
                            arguments[i] = split.Length == 3 ? traitBuildingSpot.GetField(split[2]) : traitBuildingSpot;
                            break;
